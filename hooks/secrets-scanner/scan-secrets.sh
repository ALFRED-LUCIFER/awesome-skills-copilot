...
PATTERNS=(
  ...
  # Vite frontend environment variables (VITE_ prefix exposes values to browser bundles)
  "VITE_ENV_VAR|high|VITE_[A-Z0-9_]+[[:space:]]*=[[:space:]]*['\"]?[A-Za-z0-9_/:@.~%+=-]{8,}"

  # MCP / HTTP header credentials (X-Api-Key, X-Jira-Pat, X-Confluence-Pat in mcp.json / config files)
  "MCP_API_KEY|critical|[Xx]-[Aa]pi-[Kk]ey['\"]?[[:space:]]*:[[:space:]]*['\"]?[A-Za-z0-9+/=_-]{16,}"
  "MCP_JIRA_PAT|critical|[Xx]-[Jj]ira-[Pp]at['\"]?[[:space:]]*:[[:space:]]*['\"]?[A-Za-z0-9+/=_.:@-]{16,}"
  "MCP_CONFLUENCE_PAT|critical|[Xx]-[Cc]onfluence-[Pp]at['\"]?[[:space:]]*:[[:space:]]*['\"]?[A-Za-z0-9+/=_.:@-]{16,}"
...
scan_file() {
  ...
  # Force-scan high-risk config files regardless of extension (never skip these)
  local force_scan=false
  case "$filepath" in
    *.env|*.env.*|.env|.env.*|*mcp.json|*mcp.jsonc|*.copilot/*|appsettings*.json|appsettings*.*.json)
      force_scan=true ;;
  esac
